...
void Game_Init(Game* game, int size, int player_turn, FieldType player_type_field,
    ...
    game->_batlefield.arr = ((FieldType**)malloc(size * sizeof(FieldType*)));
    game->_batlefield.size = size;
    for (int i = 0; i < size; i++) {
        game->_batlefield.arr[i] = (FieldType*)malloc(size * sizeof(FieldType));
        for (int j = 0; j < size; j++) {
            game->_batlefield.arr[i][j] = NONE;
        }
    }
}
...
FieldType CheckEnd(const Vector* const vec) {
    for (int row = 0; row < vec->size; ++row) {
        ...
        for (int col = 1; col < vec->size; ++col) {
            if (vec->arr[row][0] != vec->arr[row][col]) {
                row_win = false;
                break;
            }
        }
        if (row_win && vec->arr[row][0] != NONE) {
            return vec->arr[row][0];
        }
    }
    ...
    for (int col = 0; col < vec->size; ++col) {
        ...
        for (int row = 1; row < vec->size; ++row) {
            if (vec->arr[0][col] != vec->arr[row][col]) {
                col_win = false;
                break;
            }
        }
        if (col_win && vec->arr[0][col] != NONE) {
            return vec->arr[0][col];
        }
    }
    ...
    for (int i = 1; i < vec->size; ++i) {
        if (vec->arr[0][0] != vec->arr[i][i]) {
            diag_win = false;
            break;
        }
    }
    if (diag_win && vec->arr[0][0] != NONE) {
        return vec->arr[0][0];
    }

    diag_win = true;
    for (int i = 1; i < vec->size; ++i) {
        if (vec->arr[0][vec->size - 1] != vec->arr[i][vec->size - i - 1]) {
            diag_win = false;
            break;
        }
    }
    if (diag_win && vec->arr[0][vec->size - 1] != NONE) {
        return vec->arr[0][vec->size - 1];
    }

    for (int row = 0; row < vec->size; ++row) {
        for (int col = 0; col < vec->size; ++col) {
            if (vec->arr[row][col] == NONE) {
                return NONE;
            }
        }
    }
...
Vector CopyVector(const Vector* const vec) {
    Vector newvec = {(FieldType**)malloc(vec->size * sizeof(FieldType*)), vec->size};
    for (int i = 0; i < vec->size; ++i) {
        newvec.arr[i] = (FieldType*)malloc(vec->size * sizeof(FieldType));
        memcpy(newvec.arr[i], vec->arr[i], vec->size * sizeof(FieldType));
    }
    return newvec;
}
...
FieldType BestResult(const Vector* const vec, int m, FieldType type) {
    ...
    for (int i = 0; i < p.size; ++i) {
        for (int k = 0; k < vec->size; ++k) {
            memcpy(newvec.arr[k], vec->arr[k], vec->size * sizeof(FieldType));
        }
...
int GetStep(const Vector* const vec, int _difficulty, FieldType _player_type_field) {
    ...
    for (int i = 0; i < p.size; ++i) {
        ...
        newvec.arr[(p.arr[i] - 1) / vec->size][(p.arr[i] - 1) % vec->size] =
...
bool Step(Game* game, int step) {
    ...
    if (game->_batlefield.arr[row][col] != NONE) {
        return false;
    }
    game->_player_turn = !game->_player_turn;
    game->_batlefield.arr[row][col] = game->_turn;
